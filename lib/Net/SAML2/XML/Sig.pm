...
sub verify {
    ...
    my $signed_info_node = $self->_get_node('(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo');
    my $signed_info_node_val = $self->{parser}->findvalue('(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo');

    my $signature_node = $self->_get_node('(//saml2:Assertion|//Assertion)/dsig:Signature');
    my $signature_method =
      $self->_get_node('(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo/dsig:SignatureMethod');
    ...
    if ( defined $signature_node && ref $signature_node ) {
        ...
        $self->{dsig_prefix} =
          ( $ns->getPrefix eq '#default' ) ? '' : $ns->getPrefix;
    }
    else {
    ...
    if ( defined $self->{cert_obj} ) {
        ...
        {
            return 0
              unless $self->_verify_x509( $keyinfo_node, $signed_info_canon,
                $signature );
        }
        elsif (
            $keyinfo_node = $self->{parser}->find(
                '//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:RSAKeyValue')
          )
        {
            return 0
              unless $self->_verify_rsa( $keyinfo_node, $signed_info_canon,
                $signature );
        }
        elsif (
            $keyinfo_node = $self->{parser}->find(
        ...
        {
            return 0
              unless $self->_verify_dsa( $keyinfo_node, $signed_info_canon,
                $signature );
        }
        else {
...
'(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo/dsig:Reference/dsig:DigestMethod/@Algorithm'
      );
    my $digest = _trim(
        $self->{parser}->findvalue(
            '(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo/dsig:Reference/dsig:DigestValue')
    );

    my $signed_xml = $self->_get_signed_xml();
    my $canonical = $self->_transform( $signed_xml, $signature_node );
    my $digest_bin =
      ( $digest_method eq 'http://www.w3.org/2001/04/xmlenc#sha256' )
      ? sha256($canonical)
      : sha1($canonical);
...
sub _get_signed_xml {
    my $self = shift;
    my $id   = $self->{parser}
    ->findvalue('(//saml2:Assertion|//Assertion)/dsig:Signature/dsig:SignedInfo/dsig:Reference/@URI');
    $id =~ s/^#//;
    $self->{'sign_id'} = $id;
    my $xpath = "//*[\@ID='$id']";
    return $self->_get_node_as_text($xpath);
...
sub _find_prefixlist {
    ...
    my ($node) = @_;
    my $prefixlist =
      $self->{parser}->findvalue( 'ec:InclusiveNamespaces/@PrefixList', $node );
    return $prefixlist;
}
...
sub _verify_rsa {
    ...
    my $mod = _trim(
        $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:RSAKeyValue/dsig:Modulus'
        ) );
    my $modBin = decode_base64($mod);
    my $exp    = _trim(
        $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:RSAKeyValue/dsig:Exponent'
        ) );
...
"Crypt::OpenSSL::X509 needs to be installed so that we can handle X509 certificates"
    ...
    # Generate Public Key from XML
    my $certificate = _trim(
        $self->{parser}->findvalue(
            '//dsig:Signature/dsig:KeyInfo/dsig:X509Data/dsig:X509Certificate')
    );
...
sub _verify_x509_cert {
    ...
    if ($meth) {
        ...
        log_warn { "rsa_pub cannot $_[0]!" }
        sprintf( 'use_%s_hash', $sig_algo )
    }
...
sub _verify_dsa {
    ...
    my $p = decode_base64(
        _trim(
            $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:DSAKeyValue/dsig:P'
            ) ) );
    my $q = decode_base64(
        _trim(
            $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:DSAKeyValue/dsig:Q'
            ) ) );
    my $g = decode_base64(
        _trim(
            $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:DSAKeyValue/dsig:G'
            ) ) );
    my $y = decode_base64(
        _trim(
            $self->{parser}->findvalue(
'//dsig:Signature/dsig:KeyInfo/dsig:KeyValue/dsig:DSAKeyValue/dsig:Y'
            ) ) );
...
sub _signedinfo_xml {
    ...
    return
qq{<dsig:SignedInfo xmlns:dsig="http://www.w3.org/2000/09/xmldsig#" xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol" xmlns:xenc="http://www.w3.org/2001/04/xmlenc#">
...
sub _canonicalize_xml {
    ...
    if ( $self->{canonicalizer} eq 'XML::Canonical' ) {
        ...
        return XML::CanonicalizeXML::canonicalize( $xml, $xpath, $prefixlist,
            1, $comments );
    }
    else {
